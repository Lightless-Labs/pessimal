...
script is not. The file stays gitignored, with an empty committed placeholder; the `--config=ci`
simulator build never writes it.

## Verification ladder

- Encoder unit tests, with the golden payload taken from **what the collector accepted in the
  spike** — not from the encoder itself. A fixture generated by the thing under test cannot find a
  bug in it.
