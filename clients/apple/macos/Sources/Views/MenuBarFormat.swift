...
//  judged and already carrying its unit; the only thing left to do with it is choose how many
//  digits to show.
//

import Foundation

/// Display formatting for the records the menu renders.
///
/// `@MainActor` so the cached `RelativeDateTimeFormatter` — which is not thread-safe — is confined
/// to the actor every caller is already on.
@MainActor
enum MenuBarFormat {
    /// Shown wherever a number would be if there were one. One glyph, used everywhere, so that
    /// "no value" never gets confused with "zero".
    static let noValue = "—"
    ...
    ///
    /// `nil` latest is a real state — a series with one cumulative sample yields no rate at all —
    /// and it renders as ``noValue`` rather than as `0`.
    ...
    ///
    /// The rate suffix is chosen from the **unit**, not from `isRate` alone, because core's two
    /// rate metrics are normalised differently and share the flag: `NetworkIo` becomes bytes *per
    /// second*, while `AgentCollectionFailures` becomes the count added *since the previous
    /// sample* (`normalize::delta`, "per bucket, not per second"). A blanket `/s` would put a unit
    /// on the second one that it does not have, which is exactly the kind of confident wrong
    /// number this app exists to catch.
    ...
    /// agree about the present. Two rows disagreeing by the width of a redraw is a small lie, but
    /// it is the kind this codebase does not tell.
    ...
    ///
    /// A deadline that has passed reads "now" rather than a negative number: the poll is due, and
    /// the timer fires the moment the run loop gets to it.
